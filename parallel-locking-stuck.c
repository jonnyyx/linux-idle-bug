...
atomic_int enc_unlocked[PARALLEL_ENCODING_CORES_MAX];
...
void *encoding_worker_thread_func(void *id_arg) {
  ...
  for (;;) {
    ...
    if (ret == 0) {
      atomic_fetch_add(&enc_unlocked[thread_id], 1);
    } else {
...
void init(void)
{
	for (int i = 0; i < PARALLEL_ENCODING_CORES_MAX; ++i) {
		atomic_init(&enc_unlocked[i], 0);
	}
}
